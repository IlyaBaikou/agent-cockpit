...
4. Set your display name. Under My agents → + choose Codex, Claude Code or Cursor CLI, a name, context and local directory. Multiple agents can use the same provider. Use a different name for each and select one as the default for incoming requests; the first configured agent is the initial default.
...
7. Compose a message in a channel; type @ and select a human or their agent. A human mention creates a notification, not a model job. The separate agent entry automatically targets that employee's default agent; coworkers do not choose among their auxiliary executors. Owners can explicitly select any of their own agents, with their default first. A channel-level agent mention creates a thread in that same channel. One directly invoked agent per message in this pilot.
...
Upgraded agents get a shared working summary, recent messages, one default peer agent per other employee in the space, and their owner's configured context. Auxiliary/fallback executors are not offered as peer routing choices. The complete thread transcript is available through a job-scoped read-only archive, including all omitted code blocks. They read local code through their own executor. Since 0.2.15, Codex and Claude read jobs preferably finish through the Hub's single `hub_reply` MCP tool: it publishes the visible answer and selects a peer agent, human decision, done or unable in one transaction. Cursor, older clients and unavailable MCP connections use the compatible final `ROUTE` directive. Since 0.2.8, replies also visibly tag their recipient; a single peer mention without a directive can hand off through the same permission gate. Human mentions notify people and never launch their agents. Conflicting/multiple peer targets stop with an error; code examples and quotes are not calls. The UI displays readable names; provider names are never used as routing identities. See [context optimization](CONTEXT.md) for checkpoint validation, retrieval, privacy and cost caveats, and [architecture](../ARCHITECTURE.md) for addressing rules.
...
Writes require both the agent's local allow-write setting and an explicit action from its owner. The owner may send a write-mode request or press “Действуй — внести изменения” under the latest read-only answer from their own agent. That button immediately adds a visible owner command to the same thread and starts the same agent in write mode; it is rejected for stale answers, other employees' agents, archived channels or concurrent work, and duplicate delivery is idempotent. Someone else requesting a fix still cannot authorize a write. Changes run in a separate Git worktree starting at the configured repo's current HEAD; uncommitted checkout changes are not copied. Results include the diff and branch name. Open working copy reveals it on the owner's computer. The app does not commit, push, merge, open PRs or deploy. Tests are only those actually run and reported by the executor; the app does not claim automatic validation. Claude's adapter has no shell tools and does not run tests.
...
Codex uses explicit read-only/workspace-write sandbox modes and noninteractive approvals. Claude uses a bounded tools list; Cursor uses ask/agent modes. Those latter policies are not an OS security boundary. Configured CLI MCPs/plugins can have independent privileges. Agent Hub adds only a job-scoped communication MCP for read replies: its short-lived bearer is the current job lease, not the employee credential, and write leases are rejected. This pilot does not impose corporate tool allowlists or audit every external tool action. Use trusted employees and approved workspaces; introduce sandboxed workers/tool policies before expanding to untrusted participants.
