...
Agents get the complete thread transcript, the space's available agent directory, and their owner's configured context. They read local code through their own executor. An agent ends its answer with one ROUTE directive: a peer agent ID, a human ID, done, or unable. The UI displays readable names; provider names are never used as routing identities.
...
The additive collaboration_state table stores a versioned JSONB document. A transaction with SELECT FOR UPDATE serializes claims, completions, handoffs, member revocation and idempotent posts. SQLite uses BEGIN IMMEDIATE for the local pilot. This deliberately small pilot store is not a horizontally scalable messaging design: every sync scans and rewrites state; histories are unpaginated and serialized under one row lock. Move to normalized tables, indexed events, incremental synchronization and retention before a company-wide rollout. The transcript has a 200k-character cap; a larger thread requires a new thread with a human-supplied summary. Windows CLI argument limitations further bound an individual prompt to 24k characters in this release.
